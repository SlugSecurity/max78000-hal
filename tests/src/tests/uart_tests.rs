...
use max78000_hal::peripherals::timer::{Oscillator, Prescaler};
...
fn run_for_all_clock_configs(clock: &PeripheralHandle<Clock<TMR>>, mut f: impl FnMut()) {
    for oscillator in [Oscillator::IBRO, Oscillator::ISO] {
        for prescaler in [
            Prescaler::_1,
            Prescaler::_2,
            Prescaler::_4,
            ...
            clock.reconfigure(oscillator, prescaler).unwrap();
            f();
        }
    }
}
...
pub fn run_uart_test(
    ...
    // NO ERTCO ALLOWED
    clk0.reconfigure(Oscillator::ISO, Prescaler::_4096).unwrap();
    ...
    // ensure timer does not exist while we change the clock configs
    drop(timer);

    run_for_all_clock_configs(&clk0, || {
        // long receive test
        let mut timer = clk0.new_timer(Milliseconds(500));
        let ret = uart.recv_with_data_timeout(&mut big_buf, &mut timer);
        assert_eq!(ret, Ok(INTERJECTION.len()));
        assert_eq!(big_buf, INTERJECTION);
    });
    ...
    write!(uart, "{}", "This is a uart test, what is uart? uart is a your art. uart is universal art. uart is uncanny art.");
