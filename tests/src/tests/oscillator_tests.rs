...
fn test_iso(gcr_reg: &GCR) {
    let div = Divider::_1;
    let sys_clk = SystemClock::configure_iso(div, gcr_reg);
...
fn test_inro(gcr_reg: &GCR, trsimsir_reg: &TRIMSIR) {
    ...
    let sys_clk = SystemClock::configure_inro(freq, div, gcr_reg, trsimsir_reg);
...
fn test_ibro(gcr_reg: &GCR) {
    let div = Divider::_1;
    let sys_clk = SystemClock::configure_ibro(div, gcr_reg);
    assert_eq!(gcr_reg.clkctrl().read().sysclk_sel().is_ibro(), true);
}

fn test_ertco(gcr_reg: &GCR) {
    let div = Divider::_1;
    let sys_clk = SystemClock::configure_ertco(div, gcr_reg);
